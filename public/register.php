...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($full_name) || empty($email) || empty($password) || empty($confirm_password)) {
        ...
        if ($stmt->fetch()) {
            ...
            $stmt = $pdo->prepare("INSERT INTO users (id, full_name, email, password, role, balance) VALUES (?, ?, ?, ?, 'user', 800.00)");
            
            if ($stmt->execute([uniqid('usr_'), $full_name, $email, $hashed_password])) {
