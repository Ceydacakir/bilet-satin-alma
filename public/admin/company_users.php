    ...
    if (empty($full_name) || strlen($full_name) < 2) {
        ...
        if ($stmt->fetch()) {
            ...
            try {
                ...
                $stmt = $pdo->prepare("INSERT INTO users (id, full_name, email, password, role, company_id, balance) VALUES (?, ?, ?, ?, 'company', ?, 800.00)");
                
                if ($stmt->execute([uniqid('usr_'), $full_name, $email, $hashed_password, $company_id])) {
                        ...
                        <tbody>
                            <?php foreach ($users as $user): ?>
                                <tr>
                                    ...
                                    <td><?php echo formatDate($user['created_at'], 'd.m.Y H:m'); ?></td>
                                    <td>
    ...
    <div class="modal-dialog">
        <div class="modal-content bg-dark border-secondary">
            ...
            <div class="modal-body">
                ...
                <p class="text-muted">Bu işlem ile birlikte kullanıcının tüm firma erişimi iptal edilecek.</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">İptal</button>
